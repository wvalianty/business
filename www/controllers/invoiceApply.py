...
async def invoiceApply_index(*, keyword=None, page=1, pageSize=10):
    ...
    sql_res = ' select inc.income_id,c.name,inc.aff_date,inc.money,inv.info,inv.id invid,inv.finished,inv.finished_time from invoice inv inner join client c  on inv.client_id = c.id inner join income inc on inv.income_id = inc.id order by inc.id desc limit %s ,%s' %(limit[0],limit[1])
...
@get('/apis/finish')
async def apis_finish(*,id):  #invid
